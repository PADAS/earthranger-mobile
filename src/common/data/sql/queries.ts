...
export const INSERT_EVENT_TYPE = 'INSERT INTO event_type (remote_id, account_id, profile_id, value, display, schema, category_id, geometry_type, default_priority, icon, icon_svg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
export const UPDATE_USER_PROFILE = 'UPDATE SET username=excluded.username, pin=excluded.pin, content_type=excluded.content_type, subject_id=excluded.subject_id, permissions=excluded.permissions';
export const UPDATE_USER = 'UPDATE SET username=excluded.username, first_name=excluded.first_name, last_name=excluded.last_name, email=excluded.email, content_type=excluded.content_type, subject_id=excluded.subject_id, accepted_eula=excluded.accepted_eula, pin=excluded.pin, permissions=excluded.permissions, is_superuser=excluded.is_superuser';
...
// Upsert Statements
export const UPSERT_EVENT_TYPE_USER = `INSERT INTO event_type (remote_id, account_id, value, display, schema, category_id, geometry_type, default_priority, icon, icon_svg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (remote_id) DO ${UPDATE_EVENT_TYPE_USER}`;
export const UPSERT_EVENT_TYPE_PROFILE = `INSERT INTO event_type (remote_id, profile_id, value, display, schema, category_id, geometry_type, default_priority, icon, icon_svg) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (remote_id) DO ${UPDATE_EVENT_TYPE_PROFILE}`;
...
export const DELETE_EVENT_TYPES_BY_ID = 'DELETE from event_type WHERE event_type.remote_id = ?';
...
export const DELETE_PROFILE_BY_ID = 'DELETE FROM user_profiles WHERE remote_id = ?';
...
export const SELECT_EVENT_TYPES_BY_DISPLAY_USER = 'SELECT id, display, schema, geometry_type, icon_svg, default_priority FROM event_type JOIN (SELECT remote_id FROM event_category WHERE ( ? )) as category on category.remote_id = event_type.category_id WHERE display LIKE ? AND account_id IS NOT NULL';
...
export const SELECT_DEFAULT_EVENT_TYPE = 'SELECT display, value FROM event_type LIMIT 1';
...
export const SELECT_EVENTS_FOR_PARENT_USER = 'SELECT DISTINCT events.id, events.remote_id, events.title, events.created_at, events.updated_at, events.is_draft, event_type.icon_svg, event_type.default_priority FROM events LEFT JOIN attachments ON events.id = attachments.event_id JOIN event_type ON events.event_type_id = event_type.id JOIN (SELECT remote_id FROM event_category WHERE ( ? )) as category ON category.remote_id = event_type.category_id WHERE ( ( ? ) ) ORDER BY events.created_at DESC';
export const SELECT_EVENTS_FOR_PROFILE_USER = 'SELECT DISTINCT events.id, events.remote_id, events.title, events.created_at, events.updated_at, events.is_draft, event_type.icon_svg, event_type.default_priority FROM events LEFT JOIN attachments ON events.id = attachments.event_id JOIN event_type ON events.event_type_id = event_type.id JOIN (SELECT remote_id FROM event_category WHERE ( ? )) as category ON category.remote_id = event_type.category_id WHERE ( ( ? ) ) ORDER BY events.created_at DESC';
export const SELECT_USER_BY_REMOTE_ID = 'SELECT * FROM accounts_user WHERE remote_id = ?';
...
export const SELECT_PATROL_DETAILS_BY_PATROL_ID = 'SELECT patrols.remote_id, patrols.title, patrols.serial_number, patrol_segments.start_latitude, patrol_segments.start_longitude, patrol_segments.start_time, patrol_types.value AS patrol_type, patrol_types.icon_svg FROM patrols JOIN patrol_segments ON patrols.id = patrol_segments.patrol_id JOIN patrol_types ON patrol_segments.patrol_type_id = patrol_types.id WHERE patrol_id = ?';
...
export const SELECT_PROFILES = 'SELECT * FROM user_profiles';
export const SELECT_PROFILE_BY_REMOTE_ID = 'SELECT * FROM user_profiles WHERE remote_id = ?';
export const SELECT_PROFILE_BY_ID = 'SELECT * FROM user_profiles WHERE id = ?';
export const SELECT_PROFILE_BY_SUBJECT_ID = 'SELECT * FROM user_profiles WHERE subject_id = ?';
export const SELECT_USER_SUBJECTS = 'SELECT * FROM user_subjects';
...
export const SELECT_REPORTED_BY_USERNAME = 'SELECT subject_id, content_type FROM accounts_user WHERE username = ? UNION SELECT subject_id, content_type FROM main.user_profiles WHERE username = ?';

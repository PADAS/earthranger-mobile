...
export const SELECT_EVENTS_FOR_PARENT_USER = 'SELECT DISTINCT events.id, events.remote_id, events.title, events.created_at, events.is_draft, event_type.icon_svg, event_type.default_priority FROM events LEFT JOIN attachments ON events.id = attachments.event_id JOIN event_type ON events.event_type_id = event_type.id JOIN (SELECT remote_id FROM event_category WHERE ( ? )) as category ON category.remote_id = event_type.category_id WHERE ( ( ? ) ) ORDER BY events.created_at DESC';
